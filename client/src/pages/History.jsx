...
const History = () => {
  ...
  // Side bar show on window screen size > 1024
  useEffect(() => {
          ...
          {isSidebarOpen &&
            <motion.div
              ...
              <div className='mb-4 space-y-1'>
                <button onClick={() => navigate("/notes")} className='w-full px-3 py-2 rounded-lg text-sm text-gray-200 bg-white/10 text-start hover:white/20'>
                  ➕ New Notes
                </button>
                ...
                <ul className='space-y-3'>
                  {topics.map((t, i) => (
                    <li key={i} onClick={()=>{openNotes(t._id)}} className={`cursor-pointed rounded-xl p-3 border transition-all 
                    ${
                      activeNoteId === t._id
                    ...
                    <p className='text-sm font-semibold text-white'>{t.topic}</p>
                      ...
                      <div className='flex flex-wrap gap-2 mt-2 text-xs'>
                        ...
                        {t.classLevel && <span className='px-2 py-0.5 rounded-full bg-indigo-500/20 text-indigo-300'>class / Level : {t.classLevel}</span>}
